...
// renderCards
const renderCards = (type = "bikinitop") => {
  let content = `<div class="row ">`;
  for (item of arrProduct[type]) {
    let { name, imgSrc_jpg, imgSrc_png } = item;
    content += `
            <div class="col-3">
              <img src="${imgSrc_jpg}" alt="">
              <p>${name}</p>
              <button onclick="chonDo('${type}','${imgSrc_png}')">Thử mẫu</button>
            </div>`;
  }
  content += `</div>`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  menuItems.forEach((item) => {
    item.addEventListener("click", (event) => {
      ...
      const currentActive = document.querySelector("#menuThuDo .active");
      if (currentActive) {
